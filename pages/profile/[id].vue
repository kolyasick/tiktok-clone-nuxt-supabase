    ...
    <MainLayout>
        ...
        >
            <div class="flex w-[calc(100vw-230px)]">
                ...
                <div class="ml-5 w-full">
                    <div class="text-[30px] font-bold truncate">
                        {{ $generalStore.allLowerCaseNoCaps(authStore.user.name) }}
                    </div>
                    <div class="text-[18px] truncate">{{ authStore.user.bio }}</div>
                    <button 
                        v-if="true"
            ...
            <div class="mt-4 grid 2xl:grid-cols-6 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-3">
                    <PostUser v-for="video in authStore.user.videos" :key="video.$id" :video="video" />
            </div>
        </div>
    </MainLayout>
